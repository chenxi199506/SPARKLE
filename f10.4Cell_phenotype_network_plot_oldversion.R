...
Cell_phenotype_network_plot1  <- function(cwas.test=NULL,best.model=NULL,interaction.best.model=NULL,cell.color=c("#cd3023","#b91e45","#f9bcbd", "gray","#9BBBE1","#90C4E9","#8095CE"),cell.size=c(25,20,15,14),edge.width=c(2,3,4),arrow.size=0.75,arrow.width=1.5,edge.color=c("darkred","#b91e45","#f9bcbd",  "#9BBBE1","#4198b9","#2d4e76")){
  ...
  edges_df$weight <- ifelse(edges_df$Pvalue < 0.001, edge.width[1],
                            ifelse(edges_df$Pvalue < 0.01, edge.width[2],
                                   ifelse(edges_df$Pvalue < 0.05, edge.width[3], 0)))
  ...
  V(g)$color <-   ifelse((best.info$Pvalue < 0.001) & (best.info$Beta > 0), cell.color[1],
