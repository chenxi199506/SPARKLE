...
OR_plot_compare <- function (data, show_variable = c("Celltype", "Model", "OR(95%CI)",
                                             "SPARKLE (p-value)", "SPARKLE (p-value) adjusted","Wilicoxon (p-value)"), plot.position = 4, Tablename = "Tablename")
{

  wilicoxon_function <- function (cwas.data)
  {
  ...
  wilicox2 <- wilicoxon_function (sparkle.data)


  wilico <- wilicox2[[1]]
  wilico$celltype <- wilico$cellname
  wilico$Wilicoxon <- wilico$p.value_Non_adjusted
  wilico$sig <- ifelse(wilico$Wilicoxon<0.001,"***",
                       ifelse(wilico$Wilicoxon<0.01,"**",
                              ifelse(wilico$Wilicoxon<0.05,"*","")))

  wilico$Wilicoxon <- ifelse(wilico$Wilicoxon<0.01,"<0.01",round(wilico$Wilicoxon,2))
  wilico$Wilicoxon <- paste0(wilico$Wilicoxon,wilico$sig)
  wilico <- wilico[,c("celltype","Wilicoxon")]
  wilico$"Wilicoxon (p-value)" <- wilico$Wilicoxon
