cwas_model_cal_mix <- function (cwas.data, selectedcelltype = "selectedcelltype", 
          link = "binomial") 
{
  fitres <- NULL
  celldf <- cwas.data
  ...
  tryCatch({
    ...
    fit01 <- tryCatch(lme4::glmer(Phenotype ~ rate , family = link, data = celldf), error = function(e) NULL)
    
    fit02 <- tryCatch(lme4::glmer(Phenotype ~ rate+ Group, family = link, data = celldf), error = function(e) NULL)
    
    fit03 <- tryCatch(lme4::glmer(Phenotype ~ rate +Subgroup, family = link, data = celldf), error = function(e) NULL)
    
    fit04 <- tryCatch(lme4::glmer(Phenotype ~ rate+ Group+Subgroup, family = link, data = celldf), error = function(e) NULL)
    ...
    print(paste0("group4 ", selectedcelltype, " fitting models have been calculated successful", 
                 ""))
    fitall <- list(fit01,fit02,fit03,fit04,fit11, fit12, fit13, fit14, fit15, fit16, 
    ...
    for (i in 1:length(fitall)) {
      ...
      fitres$formula[i] <- paste(link, "link function:", 
